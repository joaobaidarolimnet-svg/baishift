...
async function registrar(bruto, o) {
  ...
  const agora = o.agora || new Date(), dia = agora.toISOString().slice(0, 10);
  const geo = await (o.geo ? o.geo(o.ip) : localizar(o.ip));
  const linha = { t: agora.toISOString(), tipo: e.tipo, pagina: e.pagina, alvo: e.alvo, origem: origem(e.ref, e.utm), utm: e.utm,
    disp: dispositivo(e.largura, o.ua), cidade: geo.cidade, uf: geo.uf, pais: geo.pais, vis: visitante(o.ip || "", o.ua || "", dia) };
