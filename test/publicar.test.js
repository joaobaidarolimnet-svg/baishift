...
function raizNova() {
  ...
  fs.mkdirSync(path.join(raiz, "apps"), { recursive: true });
  ...
  fs.writeFileSync(path.join(raiz, "outros", "severino.html"), "x");   /* endereço antigo */
...
test("modo local: materializa a imagem pendente, gera as páginas, remove órfãs e registra", async () => {
  ...
  assert.ok(!fs.existsSync(path.join(raiz, "apps", "velho.html")), "página velha removida");
  assert.ok(!fs.existsSync(path.join(raiz, "apps", "aprova-suficiencia.html")), "produto inativo sem página");
...
test("modo GitHub: commit com os arquivos certos; falha no GitHub não grava nada", async () => {
  ...
  try {
    ...
    assert.ok(caminhos.includes("conteudo/site.json") && caminhos.includes("index.html") && caminhos.includes("sitemap.xml") && caminhos.includes("apps/severino.html") && caminhos.includes("apps/index.html") && caminhos.includes("diagnostico.html") && caminhos.includes("provedores/software/painel.html"));
