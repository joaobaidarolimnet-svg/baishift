...
/* páginas que a pré-visualização sabe montar, além de "produto:<slug>" */
const PAGINAS = { hub: render.paginaHub, provedores: render.paginaProvedores, software: render.paginaSoftware,
  diagnostico: render.paginaDiagnostico, apps: render.paginaApps };

/* chega por <form method="post" target="_blank"> (urlencoded):
   conteudo=<json>&pagina=hub|provedores|software|diagnostico|apps|software:<slug>|produto:<slug> */
rota("POST", "previa", { semCabecalho: true }, async ({ res, lerCorpo }) => {
  ...
  const pagina = f.get("pagina") || "hub";
  let html;
  if (PAGINAS[pagina]) html = PAGINAS[pagina](c, { previa: true });
  else if (pagina.startsWith("software:")) {
    const p = c.software.produtos.find(x => x.slug === pagina.slice(9));
    if (!p) throw new Erro(404, "produto não encontrado no rascunho");
    html = render.paginaSoftwareProduto(p, c, { previa: true });
  } else {
    const p = c.produtos.find(x => x.slug === pagina.replace(/^produto:/, ""));
    if (!p) throw new Erro(404, "aplicativo não encontrado no rascunho");
