...
function sitemap(c) {
  ...
  const linhas = [url(HOST + "/", "1.0"), url(HOST + "/diagnostico", "0.9"),
...
function obsoletas(raiz, arquivos) {
  ...
  for (const f of ["dashboards.html", "provedores.html"])
    if (fs.existsSync(path.join(raiz, f))) fora.push(f);
  return fora;
}
...
module.exports = { paginaHub, paginaSoftware, paginaSoftwareProduto, paginaDiagnostico, paginaApps, paginaProduto, sitemap, paginas, obsoletas, limparPastaAntiga, gerarTudo, escreverAtomico };
