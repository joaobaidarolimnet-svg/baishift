...
function resumo(periodo, o = {}) {
  ...
  /* as seções do funil vivem em /provedores: a base da conversão é a visita dessa página */
  const base = paginas.filter(e => e.pagina === "/provedores").length;
  const secoes = SECOES.map(s => ({ secao: s, visitas: evs.filter(e => e.tipo === "secao" && e.alvo === s).length, base }));
  ...
  const produtos = (o.produtos || []).map(p => ({ slug: p.slug, nome: p.nome,
    /* "/outros/<slug>" e "menu:outros:<slug>" são os endereços antigos: seguem contando o histórico */
    visitas: paginas.filter(e => e.pagina === "/apps/" + p.slug || e.pagina === "/outros/" + p.slug).length,
    cliquesMenu: cliques.filter(e => e.alvo === "app:" + p.slug || e.alvo === "menu:outros:" + p.slug).length,
