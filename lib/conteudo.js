...
const SLUGS_RESERVADOS = new Set(["gestor", "api", "assets", "outros", "apps", "conteudo", "index", "provedores", "dashboards", "diagnostico", "software"]);
...
const ESQUEMA = objeto({
  ...
  hub: objeto({
    titulo: texto(LIMITES.titulo), subtitulo: multilinha(LIMITES.curto),
    chamadaMais: multilinha(LIMITES.curto),
    portas: lista(objeto({ quem: texto(LIMITES.item), titulo: texto(LIMITES.item), texto: multilinha(LIMITES.curto),
      seloDestaque: texto(40), selo: texto(LIMITES.item) }), 2, 2),
    legendaFluxo: texto(LIMITES.item),
    quemSomos: objeto({ titulo: texto(LIMITES.titulo), apoio: multilinha(LIMITES.curto),
      fatos: lista(objeto({ chave: texto(40), titulo: texto(LIMITES.item), texto: multilinha(LIMITES.curto) }), 3, 3) }),
    honestidade: objeto({ titulo: texto(LIMITES.titulo), texto: multilinha(LIMITES.curto),
      serve: lista(texto(LIMITES.item), 1, 6), naoServe: lista(texto(LIMITES.item), 1, 6) }),
    contato: objeto({ titulo: texto(LIMITES.titulo), texto: multilinha(LIMITES.curto), botaoWhatsapp: texto(LIMITES.item) })
  }),
  inicio: objeto({
    tituloAba: texto(LIMITES.titulo), descricao: texto(LIMITES.curto),
  ...
  /* as três áreas do painel do provedor (comercial, financeiro, campo) */
  painel: objeto({
    areas: lista(objeto({ nome: texto(40), rotulo: texto(LIMITES.item), titulo: texto(LIMITES.titulo),
      texto: multilinha(LIMITES.curto) }), 3, 3),
    nota: texto(LIMITES.curto)
  }),
  /* catálogo de software para provedor, em /provedores/software */
  software: objeto({
    tituloAba: texto(LIMITES.titulo), descricao: texto(LIMITES.curto),
    rotulo: texto(LIMITES.item), titulo: texto(LIMITES.titulo), lead: multilinha(LIMITES.curto),
    nota: texto(LIMITES.curto),
    comoFunciona: objeto({ rotulo: texto(LIMITES.item), titulo: texto(LIMITES.titulo),
      itens: lista(objeto({ titulo: texto(LIMITES.item), texto: multilinha(LIMITES.curto) }), 0, 6) }),
    produtos: lista(objeto({
      slug: slug(), nome: texto(LIMITES.item, { obrigatorio: true }), ativo: booleano(), demo: booleano(),
      categoria: texto(40), status: texto(LIMITES.item), cor: cor(),
      resumo: texto(LIMITES.curto), sub: multilinha(LIMITES.curto),
      titulo: texto(LIMITES.titulo), lead: multilinha(LIMITES.curto),
      beneficios: lista(objeto({ titulo: texto(LIMITES.item), texto: multilinha(LIMITES.curto) }), 0, 6),
      preco: texto(LIMITES.curto)
    }), 0, 12)
  }),
  modelos: objeto({
  ...
  apps: objeto({
    tituloAba: texto(LIMITES.titulo), descricao: texto(LIMITES.curto),
    rotulo: texto(LIMITES.item), titulo: texto(LIMITES.titulo), lead: multilinha(LIMITES.curto),
    manifesto: multilinha(LIMITES.curto),
    vazio: texto(LIMITES.curto),
    fecho: objeto({ titulo: texto(LIMITES.titulo), texto: multilinha(LIMITES.curto) })
  }),
