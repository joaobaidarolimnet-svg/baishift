...
const SLUGS_RESERVADOS = new Set(["gestor", "api", "assets", "outros", "apps", "conteudo", "index", "provedores", "dashboards"]);
...
const ESQUEMA = objeto({
  ...
  hub: objeto({
    ...
    portas: lista(objeto({ quem: texto(LIMITES.item), titulo: texto(LIMITES.item), texto: multilinha(LIMITES.curto),
      seloDestaque: texto(40), selo: texto(LIMITES.item) }), 3, 3),
  ...
  paineis: objeto({
    tituloAba: texto(LIMITES.titulo), descricao: texto(LIMITES.curto),
    rotulo: texto(LIMITES.item), titulo: texto(LIMITES.titulo), lead: multilinha(LIMITES.curto),
    segmentos: lista(objeto({ nome: texto(40), rotulo: texto(LIMITES.item), titulo: texto(LIMITES.titulo),
